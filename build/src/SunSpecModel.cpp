...
std::vector <uint16_t> SunSpecModel::PointToRegisters (
    ...
    BOOST_FOREACH (pt::ptree::value_type const& node,
        ...
        if( node.first == "point" ) {
            ...
            // TODO (TS): this should be configured by the smdx file
            if (type == "int16") {
                float value = std::stof(point[id]);
                value = value / sunssf_[scaler];
                uint16_t cast_value = value;
                std::vector <uint16_t> registers = {index, 1, cast_value};
                return registers;
            } else if (type == "uint16") {
                float value = std::stof(point[id]);
                value = value / sunssf_[scaler];
                ...
                uint32_t cast_value = value;
                SunSpecModel::SetUINT32(&registers, 2, cast_value);
                return registers;
            } else if (type == "float32") {
                float value = std::stof(point[id]);
                value = value / sunssf_[scaler];
